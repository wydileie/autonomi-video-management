...
use super::manifest::{
    manifest_original_file, manifest_variant, video_manifest_document, OriginalFileManifestInput,
    SegmentManifestInput, VariantManifestInput, VideoManifestInput,
};
use crate::{
...
pub(crate) async fn upload_approved_video_inner(
    ...
    for variant in variants {
        ...
        for input in upload_inputs {
            spawn_segment_upload(state, semaphore.clone(), &mut jobs, input);
        }
        ...
        manifest_variants.push(manifest_variant(
            variant_manifest_input(variant_id, &variant),
            segment_manifest_inputs(&uploaded_segments),
        ));
    }

    let manifest = video_manifest_document(
        video_id,
        video_manifest_input(&video_row),
        manifest_created_at,
...
async fn upload_original_file_if_needed(
    ...
    if let Some(address) = video_row
        ...
        .flatten()
    {
        return Ok(Some(manifest_original_file(
            address,
            original_file_manifest_input(video_row),
        )));
    }
    ...
    Ok(Some(manifest_original_file(
        address,
...
fn spawn_segment_upload(
    state: &AppState,
    semaphore: Arc<Semaphore>,
    jobs: &mut JoinSet<Result<SegmentUploadResult, String>>,
...
fn original_file_manifest_input(video_row: &SqliteRow) -> OriginalFileManifestInput {
    OriginalFileManifestInput {
        byte_size: video_row
...
fn variant_manifest_input(variant_id: Uuid, variant: &SqliteRow) -> VariantManifestInput {
...
fn segment_manifest_inputs(uploaded_segments: &[SqliteRow]) -> Vec<SegmentManifestInput> {
    uploaded_segments
        .iter()
        .map(|segment| SegmentManifestInput {
...
fn video_manifest_input(video_row: &SqliteRow) -> VideoManifestInput {
    VideoManifestInput {
