...
use crate::catalog::state_file::read_catalog_address;
use crate::{
    db::{db_error, parse_video_uuid},
    errors::ApiError,
    models::{SegmentOut, VariantOut, VideoOut},
    state::AppState,
    STATUS_READY,
};
...
pub(crate) fn opt_string_field(value: &Value, key: &str) -> Option<String> {
    value.get(key).and_then(Value::as_str).map(str::to_string)
}

pub(crate) fn int_field(value: &Value, key: &str) -> i32 {
