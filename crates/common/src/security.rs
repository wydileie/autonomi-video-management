...
/// Read and validate `CORS_ALLOWED_ORIGINS`-style configuration from the
/// named environment variable. Returns an empty list when unset.
pub fn cors_allowed_origins_from_env(name: &str) -> anyhow::Result<Vec<HeaderValue>> {
    match crate::env::non_empty_env(name) {
        Some(raw) => parse_cors_allowed_origins(&raw),
        None => Ok(Vec::new()),
    }
}
